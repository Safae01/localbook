  ...
  const handleDeleteComment = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Mettre à jour le nombre de commentaires dans les annonces utilisateur
        setUserAnnonces(prevAnnonces => prevAnnonces.map(annonce => {
          if (annonce.ID_POST === postId) {
            return {
              ...annonce,
              COMMENTS_COUNT: Math.max(0, (parseInt(annonce.COMMENTS_COUNT) || 0) - 1)
            };
          }
          return annonce;
        }));
                                      ...
                                      <div className="flex justify-between items-start">
                                        <div
                                          ...
                                          onClick={() => showUserProfile({
                                            ...
                                            avatar: comment.AUTHOR_AVATAR ? `http://localhost/localbook/backend/api/Uploads/users/${comment.AUTHOR_AVATAR}` : "https://via.placeholder.com/40",
                                            bio: `Profil de ${comment.AUTHOR_NAME}`,
                                            status: 'offline'
                                          })}
                                        >
                                          {comment.AUTHOR_NAME}
                                        ...
                                        {user && comment.ID_USER === user.ID_USER && (
                                          <button
                                            onClick={() => confirmDeleteComment(comment.ID_COMMENT, annonce.ID_POST)}
