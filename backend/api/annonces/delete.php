...
try {
    ...
    if (!$postId || !$userId) {
        http_response_code(400);
        echo json_encode(["error" => "postId et userId sont requis"]);
        exit;
    }
    
    // Vérifier que le post appartient bien à l'utilisateur
    $checkQuery = "SELECT ID_POST, POST_IMG, POST_VID FROM poste WHERE ID_POST = :postId AND ID_USER = :userId";
    ...
    if (!$post) {
        ...
        echo json_encode(["error" => "Post non trouvé ou vous n'avez pas l'autorisation de le supprimer"]);
        exit;
    }
